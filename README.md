...
> **EDA 핵심 발견:** 초기 가설은 RECOVERING → BROKEN 패턴 감지였으나, BROKEN 7건 전수 분석 결과 실제 순서는 **NORMAL → BROKEN → RECOVERING** 이었음. RECOVERING은 고장 전 징조가 아니라 복구 과정. EDA 없이 바로 코딩했다면 완전히 잘못된 Flink 로직을 만들었을 케이스.
